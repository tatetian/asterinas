...
(The harness itself is agent-agnostic — any agent can be run via `AGENT_PROFILE`; see [*Agent profiles*](#agent-profiles) — so this is a choice of reference, not a hard-coding.)
...
├── agent_profiles/           # per-agent launch configs (AGENT_PROFILE=<name>): claude/, codex/, …
...
Which agent **reviews *and* grades** is chosen by **`AGENT_PROFILE`** (required — bare `make benchmark` fails closed with the list of available profiles):
a profile **name** resolving to a directory under [`agent_profiles/`](../benchmark/agent_profiles/).
Two ship, both verified end-to-end (see *Harness flow*):
**`claude`** (the reference config — Claude Code, Opus, high effort) and **`codex`** (Codex, `gpt-5.5`, high effort).
...
`run.sh` makes the private `{workdir}`,
seeds the config, applies `env`,
copies `inherit`, and runs `command` with `{prompt}`
— for the reviewer and both graders alike.
...
`make benchmark AGENT_PROFILE=<name>` runs the **full suite at `MIN_RECALL=100`** on the **base** profile.
`make smoke AGENT_PROFILE=<name>` runs a **2-problem subset at `MIN_RECALL=0`** on the **`.smoke` overlay**
— tuned three ways for speed:

- **Faster config (`.smoke` overlay).**
  With `PROFILE_VARIANT=smoke`,
...
`make benchmark AGENT_PROFILE=… KEEP=<dir>` (or `KEEP=1` for a printed temp dir) copies each problem's review next to its expected defects
...
`benchmark/run.sh` runs, for the agent named by `AGENT_PROFILE`:
...
3. **Review, cheap first** — run the skill in the worktree via `AGENT_PROFILE`'s agent (`diff <base>` or `files <targets>`) with `--per-persona-context=no`;

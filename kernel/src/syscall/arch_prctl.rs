...
fn do_arch_prctl(code: ArchPrctlCode, addr: u64, ctx: &Context) -> Result<u64> {
    ...
    match code {
        ArchPrctlCode::ARCH_SET_GS => {
            ...
            tls_regs.set_gs_base(addr as usize);
            Ok(0)
        }
        ArchPrctlCode::ARCH_SET_FS => {
            ...
            tls_regs.set_fs_base(addr as usize);
            Ok(0)
        }
        ArchPrctlCode::ARCH_GET_FS => Ok(tls_regs.get_fs_base() as u64),
        ArchPrctlCode::ARCH_GET_GS => Ok(tls_regs.get_gs_base() as u64),
    }
}

...
use ostd::{
    arch::{
        cpu::context::{GeneralRegs, X86TlsRegs},
...
impl CUserRegsStruct {
    ...
    pub fn from_regs(general_regs: &GeneralRegs, tls_regs: &X86TlsRegs) -> Self {
    ...
    pub fn apply_to(
        ...
        tls_regs: &mut X86TlsRegs,
    ) -> Result<()> {
        let bytes = self.as_bytes();
        let mut new_general_regs = *general_regs;
        let mut new_tls_regs = *tls_regs;
        for rule in REG_RULES {

...
fn clone_child_task(
    ...
    // Clone FPU context
    let child_fpu_context = thread_local.supp_user_context().fpu().clone_context();
    ...
    #[cfg(target_arch = "x86_64")]
    let child_tls_regs = clone_tls_regs(thread_local, clone_flags, clone_args.tls);
    ...
    let child_task = {
        ...
        #[cfg(target_arch = "x86_64")]
        {
            thread_builder = thread_builder.tls_regs(child_tls_regs);
        }
...
fn clone_child_process(
    ...
    let child = {
        ...
        #[cfg(target_arch = "x86_64")]
        {
            child_thread_builder = child_thread_builder.tls_regs(child_tls_regs);
        }
...
#[cfg(target_arch = "x86_64")]
fn clone_tls_regs(thread_local: &ThreadLocal, clone_flags: CloneFlags, tls: u64) -> X86TlsRegs {
    let tls_regs = thread_local.supp_user_context().x86_tls_regs();
    let mut child_tls_regs = tls_regs.clone_regs();
    if clone_flags.contains(CloneFlags::CLONE_SETTLS) {
        child_tls_regs.set_fsbase(tls as usize);
    }
    child_tls_regs
}
